...
export function renderLists(): void {
  ...
  [...S.favs].sort((a, b) => a.dist - b.dist).forEach(it => {
    ...
    (row.querySelector('.ic') as HTMLElement).onclick = () => { removeFav(it.id); refresh(); renderLists(); };
  ...
  S.deleted.forEach(it => {
    ...
    (row.querySelector('.rb') as HTMLElement).onclick = () => { removeDeleted(it.id); refresh(); renderLists(); toast('復旧しました'); };
    tl.appendChild(row);
  });
}

export function initLists(): void {
  $('#btnRestoreAll').onclick = () => { if (!S.deleted.length) return; clearDeleted(); refresh(); renderLists(); toast('すべて復旧しました'); };
}
